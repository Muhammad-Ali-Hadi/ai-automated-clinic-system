import { Router } from 'express'; import rateLimit from 'express-rate-limit'; import { z } from 'zod'; import { login, logout, logoutAll, refresh, profile, updateProfile, changePassword, sessions, loginHistory, registerHospital, createUser, listUsers, updateUserRole, roles, forgotPassword, resetPassword, requestEmailVerification, verifyEmail } from '../controllers/auth.controller.js'; import { authenticate, authorize, requireTenant } from '../middlewares/auth.js'; import { validate } from '../middlewares/validate.js';
export const authRouter = Router(); const authRateLimit = rateLimit({ windowMs: 15 * 60 * 1000, limit: 10, standardHeaders: 'draft-8', legacyHeaders: false, message: { success: false, message: 'Too many authentication attempts. Please try again later.', errors: [], statusCode: 429 } }); const loginSchema = z.object({ body: z.object({ email: z.string().email(), password: z.string().min(8) }), query: z.object({}), params: z.object({}) }); const refreshSchema = z.object({ body: z.object({ refreshToken: z.string().min(1) }), query: z.object({}), params: z.object({}) }); authRouter.post('/login', authRateLimit, validate(loginSchema), login); authRouter.post('/refresh', authRateLimit, validate(refreshSchema), refresh); authRouter.post('/logout', authenticate, logout); authRouter.post('/logout-all', authenticate, logoutAll);

authRouter.get('/profile', authenticate, profile); authRouter.patch('/profile', authenticate, validate(z.object({ body: z.object({ firstName: z.string().min(1).max(100).optional(), lastName: z.string().min(1).max(100).optional() }).refine((value) => Object.keys(value).length > 0, 'At least one profile field is required'), query: z.object({}), params: z.object({}) })), updateProfile); authRouter.post('/change-password', authenticate, validate(z.object({ body: z.object({ currentPassword: z.string().min(8), newPassword: z.string().min(8) }), query: z.object({}), params: z.object({}) })), changePassword);
...
authRouter.post('/forgot-password', authRateLimit, validate(z.object({ body: z.object({ email: z.string().email() }), query: z.object({}), params: z.object({}) })), forgotPassword);
authRouter.post('/reset-password', authRateLimit, validate(z.object({ body: z.object({ token: z.string().min(1), password: passwordField }), query: z.object({}), params: z.object({}) })), resetPassword);
